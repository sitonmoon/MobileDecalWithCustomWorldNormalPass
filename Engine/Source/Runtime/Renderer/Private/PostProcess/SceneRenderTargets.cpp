...
FSceneRenderTargets::FSceneRenderTargets(const FViewInfo& View, const FSceneRenderTargets& SnapshotSource)
	...
	, CustomWorldNormal(GRenderTargetPool.MakeSnapshot(SnapshotSource.CustomWorldNormal))
...
FCustomCaptureTextures FSceneRenderTargets::RequestCustomCapture(FRHICommandListImmediate& RHICmdList, bool bPrimitives)
{
	...
	else
	{
		...
		{
			// Set render target.
			FRHIRenderPassInfo RPInfo(CustomCapture->GetTargetableRHI(), ERenderTargetActions::Clear_Store);
			RHICmdList.BeginRenderPass(RPInfo, TEXT("ClearCustomCapture"));
			//Clear RT
			RHICmdList.EndRenderPass();
		}
	}
...
FCustomWorldNormalTextures FSceneRenderTargets::RequestCustomWorldNormal(FRHICommandListImmediate& RHICmdList, bool bPrimitives)
{
	FCustomWorldNormalTextures CustomCaptureTextures{};
	...
	else
	{
		if (CustomWorldNormal)
		{
			FRHIRenderPassInfo RPInfo(CustomWorldNormal->GetTargetableRHI(), ERenderTargetActions::Clear_Store);
			RHICmdList.BeginRenderPass(RPInfo, TEXT("CustomWorldNormal"));
			RHICmdList.EndRenderPass();
		}
	}
	return CustomCaptureTextures;
	...
	FMobileSceneTextureUniformParameters& SceneTextureParameters)
{
	...
	{
		const FSceneRenderTargetItem& CaptureNormalToUse = SceneContext.CustomWorldNormal ? SceneContext.CustomWorldNormal->GetRenderTargetItem() : GSystemTextures.BlackDummy->GetRenderTargetItem();
		SceneTextureParameters.CustomWorldNormalTexture = CaptureNormalToUse.ShaderResourceTexture;
		SceneTextureParameters.CustomWorldNormalTextureSampler = TStaticSamplerState<>::GetRHI();
	}
